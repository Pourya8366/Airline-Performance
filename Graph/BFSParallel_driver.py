...
mr_job = BFSParallel(args=[INPUT_GRAPH, '--startnode', START_NODE, '--endnode', END_NODE,  '-r', 'hadoop', '--no-output', '--cleanup', 'NONE', '--no-check-input-paths', '--output-dir',  '/tmp/BFS/temp-output' ])
...
while (1):
    with mr_job.make_runner() as runner: 
        ...
        subprocess.Popen(["hadoop", "fs", "-rm", "-r", "/tmp/BFS/temp-output/"], stdout=None)
        runner.run()
        f = open(INPUT_GRAPH, 'w+')
        #cat = subprocess.Popen(["hadoop", "fs", "-cat", "/tmp/msannat/temp-output/part-00000"], stdout=subprocess.PIPE)
        cat = subprocess.run(["hadoop", "fs", "-cat", "/tmp/BFS/temp-output/part-*"], capture_output=True, text=True).stdout.split("\n")[:-1]
